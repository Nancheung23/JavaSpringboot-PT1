...
/**
 * DataLoader class is responsible for initializing the database with default data.
 * It loads predefined users, recipes, and comments into the database upon application start.
 * This class uses service layers to convert DTOs to entities before saving them to the database.
 */
@Component
public class DataLoader {
    ...
    /**
     * Constructs a new DataLoader with required repositories and conversion services.
     *
     * @param userRepository the user repository used for accessing user data.
     * @param conversionService the service for converting user DTOs to user entities.
     * @param recipeRepository the recipe repository used for accessing recipe data.
     * @param recipeConversionService the service for converting recipe DTOs to recipe entities.
     * @param commentRepository the comment repository used for accessing comment data.
     * @param commentConversionService the service for converting comment DTOs to comment entities.
     */
    ...
    /**
     * Loads predefined data into the database. This method is called automatically after the component initialization.
     * It filters users to avoid duplicates based on emails and usernames, and then saves valid users, recipes, and comments.
     *
     * @throws NoSuchAlgorithmException if there is an error during data processing, related to security algorithms.
     */
    @PostConstruct
    @Transactional
    public void loadData() throws NoSuchAlgorithmException {
        List<UserDTO> dtos = List.of(
                new UserDTO("Yannan Zhang", "nan", "admin", "http://www.google.com", "yannan.zhang@tuni.fi", "20.04.1997"),
                new UserDTO("Test User", "test1", "test1_password", "https://chatgpt.com", "testUser@test.fi", "16.09.1950"),
                new UserDTO("Test User 2", "test2", "test2_password", "http://www.youtube.com", "test@test.fi", "23.04.2021"),
                new UserDTO("Test User 3", "test3", "test3_password", "https://www.tuni.fi/en", "test_testing@test.fi", "01.01.1900")
        );
